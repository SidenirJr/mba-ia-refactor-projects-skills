"""Infraestrutura de banco: fábrica de conexão (por requisição), schema e seed.
...
def get_db():
    """Conexão associada à requisição atual (criada sob demanda)."""
    if "db" not in g:
        g.db = connect()
    return g.db
...
def init_db():
    """Cria as tabelas e popula o seed (senhas já hasheadas) se o banco estiver vazio."""
    conn = connect()
    try:
        for stmt in SCHEMA:
            conn.execute(stmt)
